...
if (window.matchMedia("(pointer: fine)").matches) {
    ...
    glow.id = 'cursor-glow';
    // Start hidden — only show after first mouse move so no dot appears on load
    glow.style.opacity = '0';
    document.body.appendChild(glow);

    let gx = -2000, gy = -2000;
    let targetGx = -2000, targetGy = -2000;
    let hasMoused = false;

    document.addEventListener('mousemove', e => {
        if (!hasMoused) {
            // Snap to cursor instantly on first move — no slide-in from corner
    ...
    // Vivid colors at full saturation — alpha controls visibility, not the color itself
    function getGlowColor() {
        const scrollY = window.scrollY;
        const maxScroll = document.body.scrollHeight - window.innerHeight;
        const progress = maxScroll > 0 ? scrollY / maxScroll : 0;

        // Wider, more gradual transitions: blue(0) → gold(0.45) → green(1.0)
        let r, g, b;
        if (progress < 0.45) {
        ...
        // Higher alpha so the color is actually readable through section backgrounds
    ...
    function animGlow() {
        ...
        // Larger radial spread so it covers more area and blends into sections
