...
function toggleTerminal() {
    ...
    if (!terminal.classList.contains('terminal-hidden')) {
        botContainer.style.opacity = '0'; botContainer.style.pointerEvents = 'none';
        setTimeout(() => termInput.focus(), 100);
    } else {
        botContainer.style.opacity = '1'; botContainer.style.pointerEvents = 'auto';
    }
}
...
function processCommand(cmd) {
    ...
    const commands = {
        ...
        'stack':   `Languages: Python · Java · C · JavaScript · SQL · Go<br>Web: React · Express · Next.js · HTML/CSS<br>Backend: Spring Boot · Supabase · MongoDB · MySQL<br>ML: scikit-learn · Pandas · NumPy · Matplotlib`,
    ...
    else if (cmd === 'github') { window.open('https://github.com/karlOrdinario', '_blank'); response.innerHTML = `Opening GitHub... <span style="color:var(--accent-green)">✓</span>`; }
    else if (commands[cmd])    { response.innerHTML = commands[cmd]; }
    else { response.innerHTML = `<span style="color:#ff6b6b">Command not found:</span> ${cmd}. Type <span style="color:var(--white)">'help'</span> for options.`; }
...
if (window.matchMedia("(pointer: fine)").matches) {
    ...
    let gx = -2000, gy = -2000, targetGx = -2000, targetGy = -2000;
    let hasMoused = false;

    document.addEventListener('mousemove', e => {
        if (!hasMoused) { gx = e.clientX; gy = e.clientY; hasMoused = true; glow.style.opacity = '1'; }
    ...
    function getGlowColor() {
        ...
        if (progress < 0.45) {
            ...
            r = Math.round(74  + (201 - 74)  * t);
            g = Math.round(158 + (168 - 158) * t);
            b = Math.round(255 + (76  - 255) * t);
        } else {
            const t = (progress - 0.45) / 0.55;
            r = Math.round(201 + (62  - 201) * t);
            g = Math.round(168 + (207 - 168) * t);
            b = Math.round(76  + (142 - 76)  * t);
        }
    ...
    function animGlow() {
        ...
        glow.style.background = `radial-gradient(700px circle at center, ${getGlowColor()}, transparent 75%)`;
        requestAnimationFrame(animGlow);
    }
    animGlow();

    document.addEventListener('mouseleave', () => { glow.style.opacity = '0'; });
    document.addEventListener('mouseenter', () => { if (hasMoused) glow.style.opacity = '1'; });
}